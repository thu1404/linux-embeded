...
int main()
{
	...
	printf("[+]Connected to connectMng.\n");
...
void showMenuAction(int deviceID)
{
	...
	while (1)
	{
		...
		while (choice == 0)
		{
			...
			printf("1. Run default setting \n");
			printf("2. Run power-saving setting\n");
			printf("3. Turn off and exit\n");
			printf("Your choice: ");
			if (kbhit())
			{
				c = getch();
...
int runDevice(int deviceID, int mode)
{
	...
	printf("Receive result from connectMng successfully\n");
	...
	if (strcmp(systemStatus, "OVER") == 0)
	{
		if (strcmp(equipStatus, "OFF") == 0)
		{
			while (countDown > 0)
			{
				printf("System overload.\nDevice will shut down in %d second.\n", countDown);
				sleep(1);
				countDown--;
				if (countDown == 0)
				{
		...
		{
			printf("HSystem overload.\nDevice will run in %s mode.\n", equipStatus);
		}
	}
	else
	{
		printf("System status is %s.\n Device is running in %s mode và %d W\n", systemStatus, equipStatus, voltage);
	}
