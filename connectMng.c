...
void powerSupply(command cmd)
{
    ...
    {
        printf("Success: Receive message from elePowerCtrl\n\n");
        ...
        printf("Success: Send result to client\n\n");
    }
...
int main()
{
    ...
    // connectMng
    // khởi tạo kết nối IP
    printf("Initalizing IP connection...\n\n");
    if ((listenSock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    {
        printf("Error occurt when initalized socket\n");
        exit(1);
    }
    ...
    {
        printf("Error occurt when binding address\n");
        exit(2);
    }
